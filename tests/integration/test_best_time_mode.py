...
class BestTimeModeApiTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_best_time_today_returns_mode_result(self) -> None:
        response = self.client.post("/decision/evaluate", json=self._decision_input("What's the best time today for a run?"))

        self.assertEqual(response.status_code, 200)
        payload = response.json()
        self.assertEqual(payload["mode_result"]["mode"], "best_time_today")
        self.assertIn("best_time", payload["mode_result"])

    def test_best_time_today_rendered_message_mentions_best_time(self) -> None:
        response = self.client.post(
            "/decision/evaluate-rendered",
            json=self._decision_input("What's the best time today for a run?"),
        )

        self.assertEqual(response.status_code, 200)
        payload = response.json()
        self.assertEqual(payload["structured"]["mode_result"]["mode"], "best_time_today")
        self.assertTrue(
            "best" in payload["rendered"]["message"].lower() or "today" in payload["rendered"]["message"].lower()
        )
